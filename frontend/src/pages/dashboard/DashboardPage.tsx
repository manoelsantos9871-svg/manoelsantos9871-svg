...
export default function DashboardPage() {
  const { data: overview, isLoading } = useQuery({ queryKey: ['dashboard-overview'], queryFn: dashboardService.getOverview });
  const { data: byStatus } = useQuery({ queryKey: ['dashboard-by-status'], queryFn: dashboardService.getDemandsByStatus });
  const { data: byType } = useQuery({ queryKey: ['dashboard-by-type'], queryFn: dashboardService.getDemandsByType });
  const { data: volume } = useQuery({ queryKey: ['dashboard-volume'], queryFn: () => dashboardService.getVolumeByPeriod(30) });
  const { data: technicians } = useQuery({ queryKey: ['dashboard-technicians'], queryFn: dashboardService.getTopTechnicians });
  const { data: avgTime } = useQuery({ queryKey: ['dashboard-avg-time'], queryFn: dashboardService.getAvgResolutionTime });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        title="Dashboard"
        subtitle="Visão geral do Departamento de Tecnologia em Saúde"
      />

      {/* Stats Grid */}
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        <StatsCard title="Total Demandas" value={overview?.totalDemands ?? 0} icon={<Ticket />} color="blue" />
        <StatsCard title="Abertas" value={overview?.openDemands ?? 0} icon={<Ticket />} color="purple" />
        <StatsCard title="Fechadas/Mês" value={overview?.closedThisMonth ?? 0} icon={<TrendingUp />} color="green" />
        <StatsCard title="SLA Violado" value={overview?.slaBreached ?? 0} icon={<AlertTriangle />} color="red" />
        <StatsCard title="Tarefas Ativas" value={overview?.pendingTasks ?? 0} icon={<CheckSquare />} color="orange" />
        <StatsCard title="OS Abertas" value={overview?.openServiceOrders ?? 0} icon={<ClipboardList />} color="purple" />
      </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1 h-1.5 rounded-full bg-gray-100 dark:bg-gray-800">
                      <div
                        ...
                        style={{ width: `${(t.resolved / (technicians[0]?.resolved || 1)) * 100}%` }}
                      />
                    </div>
                  </div>
